/**
 * Block-O Global Modal & Toast Engine
 * Modern Tailwind CSS replacement for native browser alerts, confirms, and prompts.
 * Designed with Block-O's signature Navy & Gold aesthetic.
 */

window.BlockoModal = (function () {
    function ensureContainers() {
        if (!document.getElementById('blocko-modal-container')) {
            const modalContainer = document.createElement('div');
            modalContainer.id = 'blocko-modal-container';
            modalContainer.className = 'fixed inset-0 z-[99999] hidden items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm transition-all duration-300';
        ...
        if (!document.getElementById('blocko-toast-container')) {
            ...
            toastContainer.className = 'fixed top-5 left-1/2 -translate-x-1/2 sm:left-6 sm:translate-x-0 z-[100000] flex flex-col gap-3 pointer-events-none max-w-md w-full px-4';
        ...
        return new Promise((resolve) => {
            ...
            const cancelBtnHtml = (showCancel || isPrompt) ? `
                <button type="button" id="blocko-modal-cancel-btn"
                        class="flex-1 px-5 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all">
                    ${cancelText}
                </button>
            ` : '';
            ...
            container.innerHTML = `
                <div class="relative w-full max-w-md bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-2xl p-6 transform transition-all scale-95 opacity-0 animate-modal-enter text-center" style="animation: modalEnter 0.25s ease-out forwards;">
                    ${iconHtml}
                    ...
                    <div class="mt-6 flex items-center gap-3 pt-2">
                        ...
                        <button type="button" id="blocko-modal-confirm-btn"
                                class="flex-1 px-5 py-2.5 text-sm font-bold rounded-xl transition-all ${confirmBtnClass}">
                            ${confirmText}
                        </button>
                    </div>
                </div>
    ...
    function alert(message, title = 'تنبيه', type = 'info') {
    ...
    function confirm(message, title = 'تأكيد الإجراء') {
        if (typeof message === 'object' && message !== null) {
            return showModal({ showCancel: true, type: 'warning', ...message });
        }
        return showModal({ title, message, showCancel: true, type: 'warning' });
    }

    function prompt(message, defaultValue = '', title = 'إدخال بيانات') {
        return showModal({ title, message, isPrompt: true, defaultValue, type: 'info' });
    }
...
window.showBlockoModal = window.BlockoModal.show;
window.showBlockoToast = window.BlockoModal.toast;

window._nativeAlert = window.alert;
window.alert = function (message) {
    window.BlockoModal.alert(message, 'تنبيه النظام', 'info');
};
